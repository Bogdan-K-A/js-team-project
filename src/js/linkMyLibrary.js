...
// if (filmWatched.length > 1 || filmQueue.length > 1) {
//   let allMovies = [...filmWatched, ...filmQueue];
//   console.log(allMovies);
// }

let amountCardOnPage = [];
...
// createAmountCardOnPage();
  // const firstPage = { ...amountCardOnPage };
renderTamplLibrary(r)

// renderTamplLibrary(firstPage[0].films);
}

export function createAmountCardOnPage () {
  let allObjectPage = { page: 1, films: [] };

  allMovies.forEach((el,index,array) => {
    
    allObjectPage.films.push(el);

    if (allObjectPage.films.length === 6) {
      amountCardOnPage.push(allObjectPage);
      allObjectPage = { page: allObjectPage.page + 1, films: [] };
...
function switchesLibraryPages(e) {
  ...
  createPageLibr(page);
 const nextPage = {...amountCardOnPage}
  
    refs.wrapperFilms.insertAdjacentHTML('beforeend', renderTamplLibrary(nextPage[Number(page)-1].films))
